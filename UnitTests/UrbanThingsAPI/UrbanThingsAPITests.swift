...
class UrbanThingsAPITests: XCTestCase {
    ...
    func testGetDirections() {
        ...
        let _ = sut.sendRequest(UTDirectionsRequest(
            ...
            destination: TestPlacePoint(primaryCode: "OFFICE", name:"Office", location: CLLocationCoordinate2D(latitude: 51.5291205, longitude:-0.0802295)))) { data, error in
            expectation.fulfill()
        }
    ...
    func testAppSearch() {
        let sut = UrbanThingsAPI(apiKey:ApiTestKey)
        let expectation = expectationWithDescription("Waiting for API response")
        let _ = sut.sendRequest(UTAppSearchRequest(query:"Sunnymeads Station", location: CLLocationCoordinate2D(latitude: 51.4702933, longitude:-0.5615487))) { data, error in
            expectation.fulfill()
        }
